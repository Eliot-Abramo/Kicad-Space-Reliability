...
class FieldEditorPanel(scrolled.ScrolledPanel):
    ...
    def _create_field(self, name: str, defn: dict, initial: dict[str, Any]) -> wx.Sizer:
        ...
        if ftype == "choice":
            ...
            ctrl = wx.SpinCtrlDouble(
                self, min=defn.get("min", 0), max=defn.get("max", 1000), initial=float(value or 0), inc=0.1
            )
            ctrl.SetDigits(3)
    ...
    def get_values(self) -> dict[str, Any]:
        values = {}
        try:
            for name, (ctrl, _ftype, _defn) in self.field_controls.items():
                values[name] = ctrl.GetValue()
        except Exception:  # noqa: BLE001
            pass
        return values

    def set_component_type(self, component_type: str, initial_values: dict[str, Any] | None = None):
        self.component_type = component_type
...
class ComponentEditorDialog(wx.Dialog):
    ...
    def __init__(self, parent, component: ComponentData, mission_hours: float = 43800):
        super().__init__(parent, title=f"Edit: {component.reference}", style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
    ...
    def _create_ui(self):
        ...
        self.override_val = wx.SpinCtrlDouble(
            panel, min=0, max=1e9, initial=float(init_override or 0), inc=0.1, size=dip_size(panel, 136, -1)
        )
        self.override_val.SetDigits(3)
        self.override_val.Enable(init_override is not None)
        self.override_val.Bind(wx.EVT_SPINCTRLDOUBLE, lambda e: self._update_preview())  # noqa: ARG005
        ...
        self.field_panel = FieldEditorPanel(
            panel, self.component.component_type, self.component.fields, on_change=self._update_preview
        )
    ...
    def _on_type_change(self, event):  # noqa: ARG002
        self.field_panel.set_component_type(self.type_combo.GetValue(), {})
        self._update_preview()

    def _on_override_toggle(self, event):  # noqa: ARG002
    ...
    def _update_preview(self):
        try:
            ...
            fit = result.get("fit_total", lam * FIT_PER_LAMBDA)
            r = reliability_from_lambda(lam, self.mission_hours)
            mttf_h = 1 / lam if lam > 0 else float("inf")
            mttf_y = mttf_h / 8760
            ...
            if result.get("lambda_eos", 0) > 0:
                eos_fit = result["lambda_eos"] * FIT_PER_LAMBDA
    ...
    def _on_ok(self, event):  # noqa: ARG002
...
class BatchComponentEditorDialog(wx.Dialog):
    ...
    def __init__(self, parent, components: list[ComponentData], mission_hours: float = 43800):
        super().__init__(parent, title="Batch Component Editor", style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
    ...
    def _create_ui(self):
        ...
        self.quick_override_val = wx.SpinCtrlDouble(
            right, min=0, max=1e9, initial=0, inc=0.1, size=dip_size(right, 120, -1)
        )
    ...
    def _on_quick_type(self, event):  # noqa: ARG002
        self.field_panel.set_component_type(self.type_combo.GetValue(), {})

    def _on_quick_override_toggle(self, event):  # noqa: ARG002
        enabled = self.quick_override_cb.GetValue()
        self.quick_override_val.Enable(enabled)
        self.field_panel.Enable(not enabled)

    def _on_apply_quick(self, event):  # noqa: ARG002
    ...
    def _on_edit(self, event):  # noqa: ARG002
        ...
        fields = self.results.get(comp.reference, comp.fields)
        ct = fields.get("_component_type", comp.component_type)
        edit_comp = ComponentData(reference=comp.reference, value=comp.value, component_type=ct, fields=fields)
    ...
    def _on_auto(self, event):  # noqa: ARG002
        for i, comp in enumerate(self.components):
    ...
    def _on_next_flagged(self, event):  # noqa: ARG002
...
class QuickReferenceDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        super().__init__(parent, title="IEC TR 62380 Quick Reference", style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
...
""")
        ...
        close_btn.Bind(wx.EVT_BUTTON, lambda e: self.EndModal(wx.ID_CLOSE))  # noqa: ARG005
