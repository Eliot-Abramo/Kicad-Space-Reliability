...
class KiCadTableGenerator:
    ...
    def generate_sexp(self, table: ReliabilityTable) -> str:
        ...
        lines.append(f'      (cell "")')
        lines.append(
            f'      (cell "{table.total_lambda:.2e}" (effects (font (bold yes))))'
        )
        lines.append(
            f'      (cell "{table.sheet_reliability:.4f}" (effects (font (bold yes))))'
        )
    ...
    def inject_into_schematic(
        self, sch_path: str, table: ReliabilityTable, use_text_box: bool = True
    ) -> bool:
        """
...
class ReliabilityReportGenerator:
    ...
    def generate_markdown(
        self, tables: Dict[str, ReliabilityTable], system_r: float, system_lambda: float
    ) -> str:
...
<body>
    ...
    <div class="summary">
        ...
        <p><strong>System Reliability:</strong> """
            + f"{system_r:.6f}"
            + """</p>
        <p><strong>System Failure Rate:</strong> <span class="lambda">"""
            + f"{system_lambda:.2e}"
            + """</span> failures/hour</p>
    </div>
...
if __name__ == "__main__":
    ...
    table = gen.create_table(
        "/Power/Buck/", components, total_lambda=5.35e-9, sheet_r=0.9947
    )
