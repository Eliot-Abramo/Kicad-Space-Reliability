...
class ParameterUncertaintyPanel(wx.Panel):
    ...
    def _create_ui(self):
        ...
        dist_idx = self.DISTRIBUTIONS.index(self.param.distribution) if self.param.distribution in self.DISTRIBUTIONS else 0
        ...
        cv_sizer.Add(wx.StaticText(self, label="CV:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 3)
        self.spin_cv = wx.SpinCtrlDouble(self, min=0.01, max=1.0, initial=self.param.cv, inc=0.05)
        ...
        range_sizer.Add(wx.StaticText(self, label="Range:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 3)
        self.spin_low = wx.SpinCtrlDouble(self, min=0.1, max=1.0, initial=self.param.low_mult, inc=0.1)
        ...
        range_sizer.Add(wx.StaticText(self, label="×  to"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT | wx.RIGHT, 5)
        self.spin_high = wx.SpinCtrlDouble(self, min=1.0, max=5.0, initial=self.param.high_mult, inc=0.1)
        ...
        range_sizer.Add(wx.StaticText(self, label="×"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 3)
...
class MonteCarloCalibrationDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        # Simulations
        grid.Add(wx.StaticText(panel, label="Number of simulations:"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.spin_n = wx.SpinCtrl(panel, min=100, max=100000, initial=self.config.n_simulations)
        self.spin_n.SetToolTip("More simulations = more accurate but slower")
        grid.Add(self.spin_n, 0, wx.EXPAND)
        
        # Base uncertainty
        grid.Add(wx.StaticText(panel, label="Default uncertainty (CV):"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.spin_cv = wx.SpinCtrlDouble(panel, min=0.05, max=0.5, initial=self.config.base_uncertainty_cv, inc=0.05)
        self.spin_cv.SetDigits(2)
        self.spin_cv.SetToolTip("Coefficient of variation for parameters without specific settings")
        grid.Add(self.spin_cv, 0, wx.EXPAND)
        
        # Convergence threshold
        grid.Add(wx.StaticText(panel, label="Convergence threshold:"), 0, wx.ALIGN_CENTER_VERTICAL)
        self.spin_conv = wx.SpinCtrlDouble(panel, min=0.0001, max=0.01, initial=self.config.convergence_threshold, inc=0.0005)
        self.spin_conv.SetDigits(4)
        self.spin_conv.SetToolTip("Stop early if mean changes by less than this (relative)")
        grid.Add(self.spin_conv, 0, wx.EXPAND)
        
        gen_sizer.Add(grid, 0, wx.EXPAND | wx.ALL, 10)
        
        # Check convergence checkbox
        self.chk_convergence = wx.CheckBox(panel, label="Enable early convergence detection")
        ...
        param_order = [
            "t_junction", "t_ambient", "n_cycles", "delta_t",
            "operating_power", "rated_power", "transistor_count",
            "voltage_stress_vds", "voltage_stress_vgs", "voltage_stress_vce",
            "ripple_ratio"
        ]
        ...
        for param_name in param_order:
            ...
            scroll_sizer.Add(wx.StaticLine(scroll), 0, wx.EXPAND | wx.LEFT | wx.RIGHT, 10)
    ...
    def _on_load(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            try:
                ...
                wx.MessageBox("Configuration loaded.", "Loaded", wx.OK | wx.ICON_INFORMATION)
            except Exception as e:
    ...
    def _update_ui_from_config(self):
        ...
        for name, panel in self.param_panels.items():
            ...
            dist_idx = ParameterUncertaintyPanel.DISTRIBUTIONS.index(param.distribution) if param.distribution in ParameterUncertaintyPanel.DISTRIBUTIONS else 0
            panel.cmb_dist.SetSelection(dist_idx)
