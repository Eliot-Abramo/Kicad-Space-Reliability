...
from reliability_math import (
    calculate_component_lambda, lambda_resistor, lambda_capacitor,
    lambda_integrated_circuit, lambda_diode, lambda_transistor,
    pi_thermal_cycles, pi_temperature, reliability_from_lambda,
    ActivationEnergy, RESISTOR_PARAMS, CAPACITOR_PARAMS, IC_DIE_TABLE
)
...
def original_lambda_resistor(t_ambient=25.0, operating_power=0.01, rated_power=0.125,
                              n_cycles=5256, delta_t=3.0, w_on=1.0):
...
def original_lambda_capacitor_ceramic(t_ambient=25.0, n_cycles=5256, delta_t=3.0, w_on=1.0):
...
def original_lambda_ic(t_junction=85.0, transistor_count=10000, n_cycles=5256, 
                       delta_t=3.0, w_on=1.0, construction_year=2020):
...
mine = lambda_capacitor(capacitor_type='Ceramic Class II (X7R/X5R)', **params)['fit_total']
diff_pct = abs(orig - mine) / orig * 100 if orig > 0 else 0
print(f"Original:  {orig:.4f} FIT")
print(f"My impl:   {mine:.4f} FIT")
print(f"Diff:      {diff_pct:.2f}%")
...
print("-" * 50)
params = {'t_junction': 85, 'transistor_count': 10000, 'n_cycles': 5256, 
          'delta_t': 3.0, 'construction_year': 2020}
...
print(f"My impl:   {mine:.4f} FIT")
print(f"Diff:      {diff_pct:.2f}%")
print(f"Match:     {'✓' if diff_pct < 5 else '✗'} (5% tolerance for IC due to package variations)")
...
from monte_carlo import (
    monte_carlo_components, ComponentMCInput, MonteCarloConfig,
...
for tc in verification["test_cases"]:
    ...
    print(f"  {tc['name']:<20}: nominal={tc['nominal_r']:.6f}, mc_mean={tc['mc_mean_r']:.6f}, err={tc['mean_error_pct']:.2f}% {status}")
...
test_components = [
    ...
    ComponentMCInput("C2", "Capacitor", {'t_ambient': 25, 'n_cycles': 5256, 'delta_t': 3.0, 'capacitor_type': 'Ceramic Class II (X7R/X5R)'}),
...
for n_sims in [1000, 5000, 10000]:
    ...
    rate = n_sims / elapsed
    print(f"  {n_sims:>6} sims: {elapsed:.2f}s ({rate:.0f} sims/sec) - R={result.mean:.6f}")
