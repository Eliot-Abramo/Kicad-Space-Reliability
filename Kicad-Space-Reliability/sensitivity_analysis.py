...
def generate_sobol_samples(
    d: int, n: int, bounds: List[Tuple[float, float]], seed: int = None
...
def sobol_indices(
    ...
    for i in range(d):
        ...
        S_first_conf[i] = (
            1.96 * np.sqrt(np.var(Y1 * Y1_i - term1) / N) / max(denom, 1e-15)
        )
...
class SobolAnalyzer:
    ...
    def analyze(
        ...
        for i in range(d):
            if (
                S_total[i] > 1e-6
                and interaction_scores[i] > self.interaction_threshold * S_total[i]
            ):
...
class ImportanceAnalyzer:
    ...
    def calculate_importance(
        ...
        for comp in components:
            ...
            contribution_pct = (
                (comp_lambda / total_lambda * 100) if total_lambda > 0 else 0
            )
            ...
            if connection_type == "series":
                ...
                other_r = [
                    c.get("r", 1.0) for c in components if c.get("ref") != comp_name
                ]
                r_sys_failed = 1 - np.prod([1 - r for r in other_r]) if other_r else 0

            raw = (
                system_reliability / r_sys_failed if r_sys_failed > 0 else float("inf")
            )
    ...
    def _get_derating_recommendation(
        self, comp_lambda: float, contribution_pct: float, comp_type: str
    ) -> Tuple[float, str]:
...
class ScenarioAnalyzer:
    ...
    def generate_standard_scenarios(
        self, reliability_func: Callable[[Dict[str, float]], Tuple[float, float]]
    ) -> List[WhatIfScenario]:
...
class SensitivityAnalyzer:
    ...
    def full_analysis(
        ...
        sorted_by_contrib = sorted(
            report.importance_measures, key=lambda x: -x.contribution_pct
        )
        report.critical_by_contribution = [
            im.component_name for im in sorted_by_contrib[:10]
        ]

        sorted_by_birnbaum = sorted(
            report.importance_measures, key=lambda x: -x.birnbaum
        )
        report.critical_by_birnbaum = [
            im.component_name for im in sorted_by_birnbaum[:10]
        ]

        sorted_by_raw = sorted(
            report.importance_measures, key=lambda x: -x.raw if x.raw < 1000 else 0
        )
        report.critical_by_raw = [im.component_name for im in sorted_by_raw[:10]]

        # Summary statistics
        report.components_above_threshold = sum(
            1 for im in report.importance_measures if im.contribution_pct > 5
        )
        ...
        if parameter_bounds and reliability_func:
            try:
                report.sobol = self.sobol_analyzer.analyze(
                    reliability_func, parameter_bounds, n_samples=n_sobol_samples
                )
...
def print_importance_report(measures: List[ImportanceMeasures], max_rows: int = 20):
    ...
    for im in measures[:max_rows]:
        ...
        print(
            f"{im.criticality_rank:<5} {im.component_name:<20} {im.lambda_fit:<12.2f} {im.contribution_pct:<10.1f} {im.birnbaum:<10.4f} {raw_str:<10} {im.rrw:<10.4f}"
        )
...
def print_sobol_results(result: SobolResult, max_rows: int = 15):
    ...
    print(
        "\n{:<25} {:>12} {:>12} {:>12}".format(
            "Parameter", "S_first", "S_total", "Interaction"
        )
    )
    print("-" * 70)
