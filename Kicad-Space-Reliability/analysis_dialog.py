...
from .monte_carlo import (
    ...
    MonteCarloConfig, MonteCarloEngine, monte_carlo_components_optimized
)
from .mc_calibration_dialog import MonteCarloCalibrationDialog, MCProgressDialog
import threading
from .sensitivity_analysis import (
    SobolResult, SobolAnalyzer, ImportanceAnalyzer, ImportanceMeasures,
    SensitivityAnalyzer, quick_sensitivity
)
from .reliability_math import reliability_from_lambda, calculate_component_lambda
from .report_generator import ReportGenerator, ReportData, ComponentReportData, BlockReportData
...
class AnalysisDialog(wx.Dialog):
    ...
    def _create_mc_tab(self) -> wx.Panel:
        ...
        self.btn_calibrate.SetToolTip("Configure uncertainty distributions and simulation parameters")
    ...
    def _create_contributions_tab(self) -> wx.Panel:
        ...
        ctrl.Add(wx.StaticText(ctrl_panel, label="View:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 12)
        self.contrib_view = wx.Choice(ctrl_panel, choices=["Components", "Sheets", "Top Contributors"])
        ...
        self.btn_importance = wx.Button(ctrl_panel, label="📊 Compute Importance Measures")
        ...
        self.btn_importance.SetToolTip("Calculate Birnbaum, RAW, RRW importance for all components")
        ...
        self.contrib_chart = HorizontalBarPanel(left_panel, "Failure Rate Contributions")
        ...
        self.contrib_list = wx.ListCtrl(right_panel, style=wx.LC_REPORT | wx.BORDER_SIMPLE)
        ...
        self.contrib_summary.SetFont(wx.Font(9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))
    ...
    def _on_run_mc(self, event):
        ...
        if not components:
            wx.MessageBox("No components found. Using simplified Monte Carlo.", 
        ...
        def run_mc_thread():
            ...
            try:
                def progress_cb(current, total):
                    ...
                    wx.CallAfter(progress_dlg.update, current / total * 100, 
                                f"{current}/{total}")
    ...
    def _update_mc_display(self):
        ...
        status_msg = f"Monte Carlo complete: {self.mc_result.n_simulations:,} simulations"
        if runtime > 0:
        ...
        def run_sheet_mc_thread():
            ...
            try:
                ...
                for sheet_path, data in self.sheet_data.items():
                    ...
                    wx.CallAfter(progress_dlg.update, 
                    ...
                    mc_result, lambda_samples = monte_carlo_sheet(
                        mc_components, self.mission_hours,
    ...
    def _sheet_mc_thread_complete(self, progress_dlg, n_per_sheet):
        ...
        if self._sheet_mc_thread_error:
            wx.MessageBox(str(self._sheet_mc_thread_error), "Error", wx.OK | wx.ICON_ERROR)
        ...
        self.status.SetLabel(f"Per-sheet MC complete: {completed} sheets × {n_per_sheet} simulations each")
        self._update_report()
    ...
    def _on_compute_importance(self, event):
        ...
        try:
            ...
            if len(component_lambdas) < 2:
                wx.MessageBox("Need at least 2 components for importance analysis.",
                             "Insufficient Data", wx.OK | wx.ICON_WARNING)
                return
            ...
            sorted_by_contrib = sorted(self.importance_measures.items(), key=lambda x: -x[1].contribution_pct)
            for rank, (name, measures) in enumerate(sorted_by_contrib, 1):
                measures.criticality_rank = rank
            
            self.status.SetLabel(f"Computed importance for {len(self.importance_measures)} components")
            self._update_contributions()
    ...
    def _update_contributions(self):
        ...
        view_mode = self.contrib_view.GetSelection() if hasattr(self, 'contrib_view') else 0
        ...
        for i, (name, lam, comp_type, path) in enumerate(contribs):
            ...
            if hasattr(self, 'importance_measures') and name in self.importance_measures:
