...
"""
        ...
        if data.mc_samples and len(data.mc_samples) > 10:
            histogram = self.svg.histogram(
                data.mc_samples, title="Reliability Distribution", width=700, height=280
            )
            html += f"""
...
"""
    ...
    def _generate_sensitivity_section(self, data: ReportData) -> str:
        ...
        tornado_data = []
        for name, sf, st in sorted(zip(params, s_first, s_total), key=lambda x: -x[2])[
            :10
        ]:
            impact = st * 0.1 * nominal
            tornado_data.append((name, nominal - impact, nominal, nominal + impact))

        tornado_svg = self.svg.tornado_diagram(
            tornado_data, "Parameter Sensitivity", width=700, height=350
        )
...
"""
            ...
            if block.components:
                pareto_data = [
                    (c.ref, c.lambda_fit)
                    for c in sorted(block.components, key=lambda x: -x.lambda_fit)[:15]
                ]
                pareto_svg = self.svg.pareto_chart(
                    pareto_data,
                    f"Component Contributions - {block.name}",
                    width=650,
                    height=280,
                )
                html += f"""
...
"""
    ...
    def _generate_contributions_section(self, data: ReportData) -> str:
        ...
        for block in data.blocks:
            for comp in block.components:
                all_components.append(
                    (f"{block.name}/{comp.ref}", comp.lambda_fit, comp.component_type)
                )
        ...
        pie_svg = self.svg.pie_chart(
            pie_data, "Top 10 Contributors", width=400, height=300
        )

        pareto_data = [(c[0], c[1]) for c in all_components[:20]]
        pareto_svg = self.svg.pareto_chart(
            pareto_data, "Pareto Analysis - All Components", width=750, height=320
        )
...
"""
    ...
    def generate_csv(self, data: ReportData) -> str:
        ...
        for path, sheet_data in sorted(data.sheets.items()):
            for c in sheet_data.get("components", []):
                lines.append(
                    f'"{path}","{c.get("ref","")}","{c.get("value","")}","{c.get("class","")}",{c.get("lambda",0)*1e9:.4f},{c.get("r",1.0):.8f}'
                )
        return "\n".join(lines)
