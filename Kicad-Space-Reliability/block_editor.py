...
class BlockEditor(wx.Panel):
    ...
    def screen_to_canvas(self, sx, sy):
        return int((sx - self.pan_offset[0]) / self.zoom_level), int(
            (sy - self.pan_offset[1]) / self.zoom_level
        )

    def canvas_to_screen(self, cx, cy):
        return int(cx * self.zoom_level + self.pan_offset[0]), int(
            cy * self.zoom_level + self.pan_offset[1]
        )
    ...
    def create_group(
        self, block_ids: List[str], conn_type: str, k: int = 2
    ) -> Optional[str]:
        if len(block_ids) < 2:
            return None
    ...
    def _find_position(self) -> Tuple[int, int]:
        ...
        while True:
            collision = any(
                not b.is_group and abs(b.x - x) < spacing_x and abs(b.y - y) < spacing_y
                for b in self.blocks.values()
            )
            if not collision:
                return (x, y)
            ...
            if g.is_group and g.children:
                ...
                for cid in g.children:
                    ...
                    if c:
                        ...
                        max_x, max_y = max(max_x, c.x + c.width), max(
                            max_y, c.y + c.height
                        )
                if min_x != float("inf"):
                    g.x, g.y = int(min_x - self.PAD), int(min_y - self.PAD)
                    g.width, g.height = int(max_x - min_x + 2 * self.PAD), int(
                        max_y - min_y + 2 * self.PAD
                    )
    ...
    def _edit_group(self, group_id: str):
        ...
        dlg.SetSelection(
            {"series": 0, "parallel": 1, "k_of_n": 2}.get(g.connection_type, 0)
        )
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if sel == 0:
                ...
                kdlg = wx.NumberEntryDialog(
                    self, "K value:", "K:", "K-of-N", g.k_value, 1, len(g.children)
                )
                if kdlg.ShowModal() == wx.ID_OK:
                    g.k_value = kdlg.GetValue()
                kdlg.Destroy()
                g.connection_type, g.label = (
                    "k_of_n",
                    f"{g.k_value}-of-{len(g.children)}",
                )
            self.Refresh()
            self._notify_change()
        dlg.Destroy()
