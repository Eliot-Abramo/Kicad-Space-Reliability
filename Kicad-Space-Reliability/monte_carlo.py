...
class MonteCarloResult:
    ...
    def confidence_interval(self, level: float = 0.90) -> Tuple[float, float]:
        alpha = (1 - level) / 2
        return (np.percentile(self.samples, alpha * 100), 
...
class ParameterUncertainty:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "distribution": self.distribution, "cv": self.cv,
...
class MonteCarloConfig:
    ...
    min_simulations_for_convergence: int = 500
    
    # Per-parameter uncertainty calibration
    parameter_uncertainties: Dict[str, ParameterUncertainty] = field(default_factory=dict)
    ...
    def _default_uncertainties() -> Dict[str, ParameterUncertainty]:
        ...
        return {
            ...
            "delta_t": ParameterUncertainty("delta_t", True, "triangular", 0.20, 0.5, 2.0),
            "operating_power": ParameterUncertainty("operating_power", True, "lognormal", 0.15),
            "rated_power": ParameterUncertainty("rated_power", False, "normal", 0.05),
            "voltage_stress_vds": ParameterUncertainty("voltage_stress_vds", True, "uniform", 0.10, 0.8, 1.2),
            "voltage_stress_vgs": ParameterUncertainty("voltage_stress_vgs", True, "uniform", 0.10, 0.8, 1.2),
            "voltage_stress_vce": ParameterUncertainty("voltage_stress_vce", True, "uniform", 0.10, 0.8, 1.2),
            "ripple_ratio": ParameterUncertainty("ripple_ratio", True, "lognormal", 0.30),
            "transistor_count": ParameterUncertainty("transistor_count", True, "lognormal", 0.10),
        }
    ...
    def get_uncertainty(self, param_name: str) -> ParameterUncertainty:
        ...
        return ParameterUncertainty(param_name, True, "lognormal", self.base_uncertainty_cv)
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "parameter_uncertainties": {k: v.to_dict() for k, v in self.parameter_uncertainties.items()},
        }
    ...
    def from_dict(cls, d: Dict) -> 'MonteCarloConfig':
        ...
        config = cls(**d)
        config.parameter_uncertainties = {k: ParameterUncertainty.from_dict(v) for k, v in pu.items()}
        return config
...
def sample_parameter(rng: np.random.Generator, nominal: float, 
                     uncertainty: ParameterUncertainty, n: int) -> np.ndarray:
...
def precompute_parameter_samples(
    ...
    uncertain_params = ['t_junction', 't_ambient', 'n_cycles', 'delta_t', 
                        ...
                        'voltage_stress_vce', 'ripple_ratio', 'transistor_count',
                        'rated_power']
    ...
    for comp in components:
        ...
        for param_name in uncertain_params:
            if param_name in comp.base_params:
                ...
                comp_samples[param_name] = sample_parameter(rng, nominal, uncertainty, n)
...
def monte_carlo_components_optimized(
    ...
    for sim_idx in range(n):
        ...
        if config.check_convergence and sim_idx >= config.min_simulations_for_convergence:
            if (sim_idx + 1) % 100 == 0:
                ...
                if len(convergence_history) >= 3:
                    ...
                    for i in range(1, len(last_means)):
                        if last_means[i-1] != 0:
                            rel_changes.append(abs(last_means[i] - last_means[i-1]) / abs(last_means[i-1]))
...
def monte_carlo_components(
    ...
    return monte_carlo_components_optimized(components, mission_hours, config, progress_callback)
...
def monte_carlo_sheet(
    ...
    return monte_carlo_components(mc_inputs, mission_hours, n_simulations, uncertainty_percent, seed)
...
def quick_monte_carlo(
    ...
    if components and len(components) > 0:
        ...
        if config:
            result, _ = monte_carlo_components_optimized(mc_inputs, mission_hours, config)
        else:
...
def verify_against_reference(test_cases: List[Dict] = None) -> Dict[str, Any]:
    ...
    if test_cases is None:
        test_cases = [
            {"name": "SMD Resistor", "type": "Resistor",
             "params": {'t_ambient': 25, 'operating_power': 0.01, 'rated_power': 0.125, 
                       'n_cycles': 5256, 'delta_t': 3.0, 'resistor_type': 'SMD Chip Resistor'}},
            ...
            {"name": "Digital IC", "type": "Integrated Circuit",
             "params": {'t_junction': 85, 'n_cycles': 5256, 'delta_t': 3.0,
                       'transistor_count': 10000, 'ic_type': 'Microcontroller/DSP',
    ...
    for tc in test_cases:
        ...
        mean_error = abs(mc_result.mean - nominal_r) / nominal_r * 100 if nominal_r > 0 else 0
        
        results["test_cases"].append({
            "name": tc["name"], "nominal_lambda": nominal_lambda, "nominal_r": nominal_r,
            "mc_mean_r": mc_result.mean, "mc_std_r": mc_result.std,
