...
def validate_ratio(val, name="ratio"):
    if not 0 <= val <= 1: raise ValueError(f"{name} must be 0-1: {val}")
    return val

def validate_positive(val, name="value"):
    if val < 0: raise ValueError(f"{name} must be positive")
    return val
...
class ConnectionType:
    ...
    PARALLEL = "parallel"
    K_OF_N = "k_of_n"
    def __init__(self, value=None): self._value = value or self.SERIES
    @property
    def value(self): return getattr(self, '_value', self.SERIES)
    def __eq__(self, other): return self.value == (other.value if isinstance(other, ConnectionType) else other)
    def __hash__(self): return hash(self.value)
    def __str__(self): return self.value

# === CONSTANTS ===
class ActivationEnergy:
    MOS = 3480; BIPOLAR = 4640; CAPACITOR_LOW = 1160; CAPACITOR_MED = 1740
    CAPACITOR_HIGH = 2900; ALUMINUM_CAP = 4640; RESISTOR = 1740

INTERFACE_EOS_VALUES = {
    "Not Interface": {"pi_i": 0, "l_eos": 0}, "Computer": {"pi_i": 1, "l_eos": 10},
    "Telecom (Switching)": {"pi_i": 1, "l_eos": 15}, "Telecom (Subscriber)": {"pi_i": 1, "l_eos": 70},
    "Avionics": {"pi_i": 1, "l_eos": 20}, "Power Supply": {"pi_i": 1, "l_eos": 40},
    "Industrial": {"pi_i": 1, "l_eos": 30}, "Space (LEO)": {"pi_i": 1, "l_eos": 25},
    "Space (GEO)": {"pi_i": 1, "l_eos": 35},
}

THERMAL_EXPANSION_SUBSTRATE = {"FR4 (Epoxy Glass)": 16.0, "Polyimide Flex": 6.5, "Alumina (Ceramic)": 6.5, "Aluminum (Metal Core)": 23.0}
...
IC_TYPE_CHOICES = {
    "Microcontroller/DSP": "MOS_DIGITAL", "FPGA (RAM-based)": "MOS_LCA", "CPLD/FPGA (Flash)": "MOS_CPLD",
    "Op-Amp/Comparator": "BIPOLAR_LINEAR", "LDO Regulator": "BICMOS_LOW_V", "DC-DC Controller": "BICMOS_HIGH_V",
}

IC_PACKAGE_CHOICES = {
    "SOIC-8": ("SO", 8), "SOIC-16": ("SO", 16), "TSSOP-20": ("TSSOP", 20),
    "QFP-48 (7x7mm)": ("TQFP-7x7", 48), "QFP-64 (10x10mm)": ("TQFP-10x10", 64),
    "QFN-32 (5x5mm)": ("QFN", 32, 7.07), "BGA-256": ("PBGA-17x19", 256),
}
...
DISCRETE_PACKAGE_TABLE = {
    "TO-92": {"lb": 1.0}, "TO-220": {"lb": 5.7}, "SOT-23": {"lb": 1.0}, "SOT-223": {"lb": 3.4},
    "SOD-123": {"lb": 1.0}, "DO-41": {"lb": 2.5}, "SMA": {"lb": 1.8},
    "0402": {"lb": 0.5}, "0603": {"lb": 0.6}, "0805": {"lb": 0.8}, "1206": {"lb": 1.0},
}

DIODE_BASE_RATES = {
    "Signal (<1A)": {"l0": 0.07}, "Rectifier (1-3A)": {"l0": 0.1}, "Zener": {"l0": 0.4},
    "TVS": {"l0": 2.3}, "Schottky (<3A)": {"l0": 0.15}, "LED": {"l0": 0.5},
}
...
CAPACITOR_PARAMS = {
    "Ceramic Class I (C0G)": {"l0": 0.05, "pkg_coef": 3.3e-3, "ea": ActivationEnergy.CAPACITOR_LOW, "t_ref": 303},
    ...
    "Aluminum Electrolytic": {"l0": 1.3, "pkg_coef": 1.4e-3, "ea": ActivationEnergy.ALUMINUM_CAP, "t_ref": 313},
}
...
INDUCTOR_PARAMS = {"Power Inductor": {"l0": 0.6}, "Signal Transformer": {"l0": 1.5}, "Power Transformer": {"l0": 3.0}}
...
def lambda_eos(is_interface: bool, interface_type: str = "Not Interface") -> float:
    if not is_interface: return 0.0
...
def calculate_ic_lambda3(pkg_type: str, pins: int = None, diag: float = None) -> float:
    ...
    if not pkg: return 4.0
    f = pkg.get("formula", "fixed")
    if f == "fixed": return pkg["value"]
    elif f == "pins" and pins: return pkg["coef"] * (pins ** pkg["exp"])
    elif f == "diagonal" and diag: return pkg["coef"] * (diag ** pkg["exp"])
    return 4.0

# === COMPONENT CALCULATIONS ===
def lambda_integrated_circuit(ic_type="MOS_DIGITAL", transistor_count=10000, construction_year=2020,
                               t_junction=85.0, package_type="TQFP-10x10", pins=48, substrate_alpha=16.0,
                               package_alpha=21.5, is_interface=False, interface_type="Not Interface",
                               n_cycles=5256, delta_t=3.0, tau_on=1.0, **kw) -> Dict[str, float]:
    ...
    return {"lambda_die": lambda_die*1e-9, "lambda_package": lambda_pkg*1e-9, "lambda_eos": lambda_e*1e-9,
            "lambda_total": total, "fit_total": lambda_die + lambda_pkg + lambda_e, "pi_t": pi_t, "pi_n": pi_n, "pi_alpha": pi_a}

def lambda_diode(diode_type="Signal (<1A)", t_junction=85.0, package="SOD-123", is_interface=False,
                 interface_type="Not Interface", n_cycles=5256, delta_t=3.0, tau_on=1.0, **kw) -> Dict[str, float]:
    tau_on = validate_ratio(tau_on, "tau_on")
...
def lambda_transistor(transistor_type="Silicon MOSFET (â‰¤5W)", t_junction=85.0, package="SOT-23",
    ...
    total = (lambda_die + lambda_pkg + lambda_e) * 1e-9
    return {"lambda_die": lambda_die*1e-9, "lambda_package": lambda_pkg*1e-9, "lambda_eos": lambda_e*1e-9,
            "lambda_total": total, "fit_total": lambda_die + lambda_pkg + lambda_e, "pi_s": pi_s}

def lambda_capacitor(capacitor_type="Ceramic Class II (X7R/X5R)", t_ambient=25.0, ripple_ratio=0.0,
                      n_cycles=5256, delta_t=3.0, tau_on=1.0, **kw) -> Dict[str, float]:
    tau_on = validate_ratio(tau_on, "tau_on")
    ...
    t_op = t_ambient + 20 * (ripple_ratio ** 2) if "Aluminum" in capacitor_type and ripple_ratio > 0 else t_ambient
    ...
    return {"lambda_base": lambda_base*1e-9, "lambda_package": lambda_pkg*1e-9, "lambda_total": total,
            "fit_total": lambda_base + lambda_pkg, "pi_t": pi_t, "pi_n": pi_n}

def lambda_resistor(resistor_type="SMD Chip Resistor", t_ambient=25.0, operating_power=0.01,
                    rated_power=0.125, n_resistors=1, n_cycles=5256, delta_t=3.0, tau_on=1.0, **kw) -> Dict[str, float]:
    tau_on = validate_ratio(tau_on, "tau_on")
    ...
    total = (lambda_base + lambda_pkg) * 1e-9
    return {"lambda_base": lambda_base*1e-9, "lambda_package": lambda_pkg*1e-9, "lambda_total": total,
            "fit_total": lambda_base + lambda_pkg, "t_resistor": t_r, "pi_t": pi_t}

def lambda_inductor(inductor_type="Power Inductor", t_ambient=25.0, power_loss=0.1,
                    surface_area_mm2=100.0, n_cycles=5256, delta_t=3.0, tau_on=1.0, **kw) -> Dict[str, float]:
    tau_on = validate_ratio(tau_on, "tau_on")
    ...
    total = (lambda_base + lambda_pkg) * 1e-9
    return {"lambda_base": lambda_base*1e-9, "lambda_package": lambda_pkg*1e-9, "lambda_total": total,
            "fit_total": lambda_base + lambda_pkg, "t_component": t_c}

def lambda_misc_component(component_type, n_contacts=1, n_cycles=5256, delta_t=3.0, tau_on=1.0, **kw) -> Dict[str, float]:
    tau_on = validate_ratio(tau_on, "tau_on")
    base = MISC_COMPONENT_RATES.get(component_type, 10.0)
    if "Connector" in component_type: base *= n_contacts
    pi_n = pi_thermal_cycles(n_cycles)
    total = base * (tau_on + 3e-3 * pi_n * (delta_t ** 0.68)) * 1e-9
    return {"lambda_total": total, "fit_total": total * 1e9}

# === SYSTEM RELIABILITY ===
def reliability_from_lambda(lam: float, hours: float) -> float: return math.exp(-lam * hours)
def lambda_from_reliability(r: float, hours: float) -> float:
    if r <= 0: return float('inf')
    if r >= 1: return 0.0
    return -math.log(r) / hours
def mttf_from_lambda(lam: float) -> float: return float('inf') if lam <= 0 else 1.0 / lam
def r_series(r_list: List[float]) -> float:
    result = 1.0
    for r in r_list: result *= r
    return result
def r_parallel(r_list: List[float]) -> float:
    p_fail = 1.0
    for r in r_list: p_fail *= (1 - r)
    return 1.0 - p_fail
def r_k_of_n(r_list: List[float], k: int) -> float:
    n = len(r_list)
    if k > n or k < 1: return 0.0
    if k == 1: return r_parallel(r_list)
    if k == n: return r_series(r_list)
    if len(set(r_list)) == 1:
        r = r_list[0]
        return sum(math.comb(n, i) * (r ** i) * ((1 - r) ** (n - i)) for i in range(k, n + 1))
    return r_list[-1] * r_k_of_n(r_list[:-1], k - 1) + (1 - r_list[-1]) * r_k_of_n(r_list[:-1], k)
def lambda_series(lam_list: List[float]) -> float: return sum(lam_list)

# === FIELD DEFINITIONS ===
def get_component_types() -> List[str]:
    return ["Integrated Circuit", "Diode", "Transistor", "Capacitor", "Resistor", "Inductor/Transformer", "Miscellaneous"]
...
def get_field_definitions(ct: str) -> Dict[str, Dict]:
    ...
    if ct == "Integrated Circuit":
        return {"ic_type": {"type": "choice", "choices": list(IC_TYPE_CHOICES.keys()), "default": "Microcontroller/DSP", "required": True},
                "transistor_count": {"type": "int", "default": 10000, "required": True},
                "t_junction": {"type": "float", "default": 85.0, "required": True},
                "package": {"type": "choice", "choices": list(IC_PACKAGE_CHOICES.keys()), "default": "QFP-48 (7x7mm)", "required": True},
                "substrate": {"type": "choice", "choices": list(THERMAL_EXPANSION_SUBSTRATE.keys()), "default": "FR4 (Epoxy Glass)"},
                **iface, **common}
    elif ct == "Capacitor":
        return {"capacitor_type": {"type": "choice", "choices": list(CAPACITOR_PARAMS.keys()), "default": "Ceramic Class II (X7R/X5R)", "required": True},
                "t_ambient": {"type": "float", "default": 25.0, "required": True}, **common}
    elif ct == "Resistor":
        return {"resistor_type": {"type": "choice", "choices": list(RESISTOR_PARAMS.keys()), "default": "SMD Chip Resistor", "required": True},
                "t_ambient": {"type": "float", "default": 25.0, "required": True},
                "operating_power": {"type": "float", "default": 0.01, "required": True},
                "rated_power": {"type": "float", "default": 0.125, "required": True}, **common}
    return {"component_subtype": {"type": "choice", "choices": list(MISC_COMPONENT_RATES.keys()), "default": "Crystal Oscillator (XO)"}, **common}
...
def calculate_component_lambda(ct: str, params: Dict[str, Any]) -> Dict[str, float]:
    if ct == "Integrated Circuit":
        ...
        return lambda_integrated_circuit(ic_type=ic_key, transistor_count=params.get("transistor_count", 10000),
                                          t_junction=params.get("t_junction", 85.0), package_type=pkg_info[0], pins=pkg_info[1] if len(pkg_info)>1 else 48,
                                          substrate_alpha=sub, is_interface=params.get("is_interface", False),
                                          interface_type=params.get("interface_type", "Not Interface"),
                                          n_cycles=params.get("n_cycles", 5256), delta_t=params.get("delta_t", 3.0), tau_on=params.get("tau_on", 1.0))
    elif ct == "Diode": return lambda_diode(**params)
    elif ct == "Transistor": return lambda_transistor(**params)
    elif ct == "Capacitor": return lambda_capacitor(**params)
    elif ct == "Resistor": return lambda_resistor(**params)
    elif ct == "Inductor/Transformer": return lambda_inductor(**params)
...
def calculate_lambda(cls: str, params: Dict = None) -> float:
    ...
    c = cls.lower()
    if "resistor" in c: return lambda_resistor(**params)["lambda_total"]
    if "capacitor" in c: return lambda_capacitor(**params)["lambda_total"]
    if "transistor" in c or "mosfet" in c: return lambda_transistor(**params)["lambda_total"]
    if "diode" in c: return lambda_diode(**params)["lambda_total"]
    if "ic" in c or "integrated" in c: return lambda_integrated_circuit(**params)["lambda_total"]
    return 10e-9

# === UTILITIES ===
def fit_to_lambda(fit): return fit * 1e-9
def lambda_to_fit(lam): return lam * 1e9
def format_lambda(lam, as_fit=True): return f"{lam*1e9:.2f} FIT" if as_fit else f"{lam:.2e} /h"
def format_reliability(r): return f"{r:.6f}" if r >= 0.9999 else f"{r:.4f}" if r >= 0.99 else f"{r:.3f}"
