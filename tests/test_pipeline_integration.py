...
class ClassificationToLambdaIntegrationTests(unittest.TestCase):
...
class LambdaToBudgetIntegrationTests(unittest.TestCase):
    def test_sheet_data_flows_to_budget(self):
        ...
        self.assertAlmostEqual(result.actual_fit, 50.0)
        self.assertGreater(result.target_fit, 0)
        self.assertEqual(len(result.sheet_budgets), 1)
    ...
    def test_budget_with_different_strategies_yield_different_allocations(self):
        ...
        self.assertGreater(budgets_prop[0], budgets_prop[1])


class PipelineToSnapshotIntegrationTests(unittest.TestCase):
    def test_sheet_data_to_snapshot(self):
        ...
        self.assertEqual(snap.n_components, 2)
        self.assertEqual(snap.n_sheets, 1)
        self.assertAlmostEqual(snap.system_lambda, 50e-9)
        self.assertAlmostEqual(snap.system_fit, 50.0)
    ...
    def test_snapshot_roundtrip_via_json(self):
        ...
        for attr in ("version_label", "n_components", "system_lambda", "system_fit", "mission_hours"):
            self.assertEqual(getattr(snap, attr), getattr(restored, attr))


class LambdaConsistencyTests(unittest.TestCase):
    def test_reliability_is_monotonic_decreasing_with_lambda(self):
        ...
        for i in range(len(rs) - 1):
            self.assertGreater(rs[i], rs[i + 1])
    ...
    def test_fit_conversion_via_budget_allocation(self):
        ...
        self.assertAlmostEqual(result.actual_fit, expected_fit)
