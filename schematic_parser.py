...
class SchematicParser:
    ...
    def parse(self) -> bool:
        """Parse the schematic hierarchy. Returns True on success."""
    ...
    def _parse_sheet(self, sch_path: Path, hierarchy_path: str):
        """Parse a single sheet recursively."""
    ...
    def _extract_properties(self, sexp: str) -> Dict[str, str]:
        """Extract properties from S-expression."""
        props = {}
...
def create_test_data(sheet_names: List[str]) -> SchematicParser:
    ...
    for path in sheet_names:
        ...
        # Add mock components based on sheet name
        if "Power" in path or "LDO" in path or "Buck" in path or "Charger" in path:
            sheet.components = [
                Component("R1", "10k", "Device:R", path, "0603", 
                         {"Reliability_Class": "Resistor", "Operating_Power": "0.01"}),
                Component("R2", "4.7k", "Device:R", path, "0603",
                         {"Reliability_Class": "Resistor"}),
                Component("C1", "100n", "Device:C", path, "0603",
                         {"Reliability_Class": "Ceramic Capacitor"}),
                Component("C2", "10u", "Device:C_Polarized", path, "0805",
                         {"Reliability_Class": "Tantalum Capacitor"}),
                Component("U1", "LM7805", "Regulator_Linear:LM7805", path, "TO-220",
                         {"Reliability_Class": "LDO Regulator", "T_Junction": "100"}),
            ]
        elif "MCU" in path:
            sheet.components = [
                Component("U1", "STM32F4", "MCU:STM32F4", path, "TQFP-100",
                         {"Reliability_Class": "Integrated Circuit", "T_Junction": "85",
                          "Package": "TQFP-100", "IC_Type": "MOS Standard, Digital circuits, 20000 transistors"}),
                Component("Y1", "8MHz", "Device:Crystal", path, "",
                         {"Reliability_Class": "Crystal"}),
                Component("C1", "20p", "Device:C", path, "0402",
                         {"Reliability_Class": "Ceramic Capacitor"}),
                Component("C2", "20p", "Device:C", path, "0402",
                         {"Reliability_Class": "Ceramic Capacitor"}),
            ]
        elif "Trigger" in path or "Logic" in path:
            sheet.components = [
                Component("U1", "74HC00", "Logic:74HC00", path, "SOIC-14",
                         {"Reliability_Class": "Integrated Circuit", "Package": "SOIC-16"}),
                Component("R1", "10k", "Device:R", path, "0402",
                         {"Reliability_Class": "Resistor"}),
            ]
        else:
            sheet.components = [
                Component("R1", "1k", "Device:R", path, "0603",
                         {"Reliability_Class": "Resistor"}),
                Component("C1", "100n", "Device:C", path, "0603",
                         {"Reliability_Class": "Ceramic Capacitor"}),
            ]
